...
class DataLoader:
    ...
    def start(self):
        try:
            dirs: Set[Path] = self.get_paths(self.source_path)
            for d in dirs:
                if d and d.is_dir():
                    dn = d.name
                    subs: Set[Path] = self.get_paths(str(d))
                    for s in subs:
                        if s and s.is_dir():
                            sn = s.name
                            self.process(s, dn, sn)
    ...
    def process(self, s: Path, dn: str, sn: str):
        if self.is_valid_dir(str(s)):
            self.process_datasets(s, dn, sn)
            self.process_orgs(s, dn, sn)
            self.process_asns(s, dn, sn)
            self.process_links(s, dn, sn)
            self.process_cones(s, dn, sn)
            self.process_locations(s, dn, sn)
            self.process_prefixes(s, dn, sn)

    def process_datasets(self, s: Path, dn: str, sn: str):
        ms = 'dataset'
        timestamp = datetime.strptime(dn, self.DF)
        datapoints = self.get_dir_file_gen(s, dn, ms)
        ipv = IP_TYPE.get(sn, 4)
        processed = set()
        elems = []
        try:
    ...
    def process_orgs(self, s: Path, dn: str, sn: str):
        ms = 'organizations'
        timestamp = datetime.strptime(dn, self.DF)
        datapoints = self.get_dir_file_gen(s, dn, ms)
        ipv = IP_TYPE.get(sn, 4)
        grouped_countries = self.grouping_countries_by_iso(self.get_dir_file_gen(s, dn, "countries"))
        processed = set()
        elems = []
        try:
            for dp in datapoints:
                ...
                el = {'org_id': oid,
                      ...
                      'ip_version': int(dp.get('ipVersion', ipv)),
    ...
    def process_asns(self, s: Path, dn: str, sn: str):
        ms = 'asns'
        timestamp = datetime.strptime(dn, self.DF)
        datapoints = self.get_dir_file_gen(s, dn, ms)
        ipv = IP_TYPE.get(sn, 4)
        orgs = self.grouping_orgs_by_id(self.get_dir_file_gen(s, dn, 'organizations'))
        countries = list(self.get_dir_file_gen(s, dn, 'countries'))
        processed = set()
        elems = []
        try:
            for dp in datapoints:
    ...
    def process_cones(self, s: Path, dn: str, sn: str):
        ms = 'cones'
        ipv = IP_TYPE.get(sn, 4)
        timestamp = datetime.strptime(dn, self.DF)
        datapoints = self.get_dir_file_gen(s, dn, 'asnCones')
        asns = self.grouping_asns_by_id(self.get_dir_file_gen(s, dn, 'asns'))
        prefs = self.grouping_prefs_by_id(self.get_dir_file_gen(s, dn, 'prefixCones'))
        processed = set()
        elems = []
        try:
            for dp in datapoints:
    ...
    def process_prefixes(self, s: Path, dn: str, sn: str):
        ms = 'prefixes'
        ipv = IP_TYPE.get(sn, 4)
        timestamp = datetime.strptime(dn, self.DF)
        datapoints = self.get_dir_file_gen(s, dn, 'prefixCones')
        asns = self.grouping_asns_by_id(self.get_dir_file_gen(s, dn, 'asns'))
        processed = set()
        elems = []
        try:
    ...
    def process_links(self, s: Path, dn: str, sn: str):
        ms = 'links'
        ipv = IP_TYPE.get(sn, 4)
        timestamp = datetime.strptime(dn, self.DF)
        datapoints = self.get_dir_file_gen(s, dn, 'asnLinks')
        asns = self.grouping_asns_by_id(self.get_dir_file_gen(s, dn, 'asns'))
        countries = list(self.get_dir_file_gen(s, dn, 'countries'))
        processed = set()
        elems = []
    ...
    def process_locations(self, s: Path, dn: str, sn: str):
        ms = 'locations'
        ipv = IP_TYPE.get(sn, 4)
        timestamp = datetime.strptime(dn, self.DF)
        datapoints = self.get_dir_file_gen(s, dn, ms)
    ...
    def get_paths(self, source: str) -> Set[Path]:
        result = set()
        try:
            if self.is_valid_dir(source):
                path = Path(source)
                result = sorted([d for d in path.iterdir() if path.is_dir()])
        except Exception as e:
            logger.info(e)
        return result
    ...
    @staticmethod
    def get_dir_file_gen(d: Path, dn: str, ms: str) -> Iterator[dict]:
        try:
            if d.exists():
                fn = d.joinpath('{dir}.{name}.{ext}'.format(dir=dn, name=ms, ext='jsonl'))

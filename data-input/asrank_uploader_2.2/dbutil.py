            ...
            try:
                ...
                with open(os.path.join(options['SQL_PATH'], dn), 'rt') as sql:
                    ...
                    with self.conn.cursor() as cursor:
                        ...
                        print("{}Create table {} {}".format(color['purple'], table_name, color['reset']), file=sys.stdout)
            except Exception as e:
                print("{} {} {}".format(color['red'], e, color['reset']), file=sys.stderr)
                logger.error(e)
                exit(1)
        ...
        if self.table_exist(table_name):
            try:
                ...
                with self.conn.cursor() as cursor:
                    ...
                    print("{}Drop table {} {}".format(color['purple'], table_name, color['reset']), file=sys.stdout)
            except Exception as e:
                print("{} {} {}".format(color['red'], e, color['reset']), file=sys.stderr)
                logger.error(e)
                exit(1)
        ...
        try:
            ...
            with conn.cursor() as cursor:
                if not self.db_exist(conn, self.opt['database']):
                    cursor.execute('CREATE DATABASE {};'.format(self.opt['database']))
